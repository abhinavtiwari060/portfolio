...
I strongly believe that good software is not just about writing code that works—it is about writing code that is simple to understand, straightforward to maintain, and a pleasure for end users to interact with every day.`;
  ...
  const learningNow = profile.learningNow && profile.learningNow.length > 0
    ? profile.learningNow
    : [
  ...
  return (
    <section id="about" className="py-24 relative overflow-hidden">
      ...
      <div className="absolute top-1/2 right-0 w-96 h-96 bg-orange-600/10 rounded-full blur-[120px] pointer-events-none" />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <ClayCard className="p-8 sm:p-12 border border-white/5 bg-gradient-to-br from-charcoal-850/90 to-charcoal-900/90 backdrop-blur-xl">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-14">
            ...
            <div className="lg:col-span-7 flex flex-col justify-between">
              <div>
                ...
                <div className="prose prose-invert max-w-none text-charcoal-300 leading-relaxed space-y-4 text-base">
                  {detailedBio.split("\n\n").map((paragraph, idx) => (
              ...
              <div className="mt-8 p-6 rounded-2xl bg-charcoal-950/70 border border-orange-500/20 shadow-clay-pill">
                <div className="flex items-center gap-2.5 text-xs font-bold uppercase tracking-wider text-orange-400 mb-2">
                  <Compass className="w-4 h-4" />
                  Developer Philosophy
                </div>
                <p className="text-sm font-medium text-charcoal-200 italic leading-relaxed">
                  "{devPhilosophy}"
                </p>
              </div>
            </div>
            ...
            <div className="lg:col-span-5 flex flex-col gap-8">
              ...
              <div className="p-6 rounded-2xl bg-charcoal-800/60 border border-white/5">
                <h4 className="text-base font-bold text-white flex items-center gap-2 mb-3">
                  <Cpu className="w-4 h-4 text-orange-400" />
                  What I Like Building
                </h4>
                <p className="text-sm text-charcoal-300 leading-relaxed">{whatILikeBuilding}</p>
              </div>
              ...
              <div className="p-6 rounded-2xl bg-charcoal-800/60 border border-white/5">
                ...
                <ul className="space-y-2">
                  {learningNow.map((item, idx) => (
                    <li key={idx} className="flex items-center gap-2.5 text-xs sm:text-sm text-charcoal-300">
                ...
                {education.map((edu, idx) => (
                  <div key={idx} className="space-y-1">
                    <div className="flex items-start justify-between gap-2">
                      <h5 className="text-sm font-bold text-white leading-snug">{edu.degree}</h5>
                      <span className="text-xs font-semibold px-2.5 py-0.5 rounded-full bg-charcoal-750 text-orange-400 border border-white/5 shrink-0">
                        {edu.year}
                      </span>
                    </div>
                    <p className="text-xs font-medium text-charcoal-300">{edu.institution}</p>
                    {edu.description && (
              ...
              <div className="p-6 rounded-2xl bg-charcoal-800/60 border border-white/5">
                ...
                <p className="text-sm text-charcoal-300 leading-relaxed">{careerGoals}</p>
              </div>
            </div>
          </div>
        </ClayCard>
